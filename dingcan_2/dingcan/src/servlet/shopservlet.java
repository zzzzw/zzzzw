...
public class shopservlet extends HttpServlet 
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(action.equals("delete2")){
			String a=request.getParameter("xuhao");
			String sql="delete from cart where xuhao='"+a+"'";
			try {
				DBbean.executesql(sql);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
		...
		if(action.equals("cart")){
			...
			sql=sql+"'"+a+"'"+","+"'"+b+"'"+","+"'"+c+"'"+","+"'"+d+"'"+")";
			String sql2="select * from cart where xuhao='"+a+"'";
			String sql3="update cart set shuliang =shuliang +"+f+" where xuhao='"+a+"'";
			try {
				ResultSet rs=DBbean.getResultSet(sql2);
				if(rs.next()){
					DBbean.executesql(sql3);
				}
				else{
					DBbean.executesql(sql);
				}
		...
		if(action.equals("upordown"))
		{
			...
			try{
				if(b.equals("下架"))
				{
					sql2="update shop set zhuangtai='上架' where  xuhao='"+a+"'";
